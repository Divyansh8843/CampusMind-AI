...
def _fallback_interview_reply(history: list, user_response: str, topic: str):
    topic = topic or "Computer Science Fundamentals"
    if not user_response.strip():
        return {
            "response": (
                f"Let's begin the {topic} interview. Explain the most important concept in {topic} "
                "that every strong candidate should know, then give one practical example."
            )
        }

    return {
        "response": (
            "Your answer has a reasonable direction, but tighten the structure, mention trade-offs, "
            f"and justify your choices more clearly. Next question: in {topic}, what would break at scale "
            "and how would you redesign it?"
        )
    }


def _fallback_aptitude_test(topic: str):
    safe_topic = topic or "General Aptitude"
    return [
        {
            "id": index + 1,
            "question": f"{safe_topic}: Practice question {index + 1}. Choose the strongest answer based on fundamentals and reasoning.",
...
def _extract_json_payload(content: str, fallback):
    cleaned = (content or "").strip().replace("```json", "").replace("```", "")
    try:
...
def conduct_interview(history: list, user_response: str, topic: str = "General Engineering"):
    ...
    try:
        llm_engine = get_llm()
        if not llm_engine:
             return _fallback_interview_reply(history, user_response, topic)
        ...
        print(f"Interview fallback triggered: {type(e).__name__}")
        return _fallback_interview_reply(history, user_response, topic)
...
def generate_aptitude_test(topic: str):
    ...
    try:
        llm_engine = get_llm()
        if not llm_engine:
             return _fallback_aptitude_test(topic)
...
def generate_interview_feedback(history: list, topic: str, user_transcript: str = ""):
    ...
    try:
        ...
        if not llm_engine:
             ...
             answer_count = len([msg for msg in history if msg.get("role") == "user" and msg.get("content")])
             score = min(85, 45 + (answer_count * 8) - (voice_analysis.get("filler_count", 0) * 2))
             return {
                 "score": max(25, score),
                 "feedback": "Fallback interview review generated because the live AI model is unavailable.",
                 "strengths": [
                     "Completed the interview flow and attempted multiple answers.",
                     "Stayed on topic for the selected interview domain.",
                 ],
                 "improvements": [
                     "Add more structured examples and clearer trade-off discussion.",
                     "Use shorter, more confident answers with measurable outcomes.",
                 ],
        ...
        out = _extract_json_payload(
            ...
            {
                "score": 0,
                "feedback": "Could not parse structured feedback from the AI interviewer.",
                "strengths": [],
                "improvements": [],
            },
        )
