...
const sendJobAlertEmail = async (email, jobsList) => {
    if (!process.env.EMAIL_USER || !jobsList || jobsList.length === 0) return;
    try {
        const jobCards = jobsList.slice(0, 5).map(job => `
            <div class="job-card">
        ...
        await transporter.sendMail({
            ...
            to: email,
            subject: `🚀 ${jobsList.length} New Job Opportunities Detected!`,
            html: `
                <!DOCTYPE html>
                <html>
                <head>
                    ...
                    <style>
                        ...
                        .tag { display: inline-block; background-color: #e0f2fe; color: #0284c7; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 600; margin-right: 5px; margin-bottom: 5px; }
                        .cta-button { display: inline-block; margin-top: 15px; padding: 10px 16px; background-color: #2563eb; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 14px; transition: background-color 0.3s; }
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            <h2 style="color: #2563eb; margin-top: 0;">New Opportunities Detected 🚀</h2>
                            <p>Hello,</p>
                            <p>Our intelligent AI agents have just discovered new career opportunities that match your profile. Here are the details:</p>
                            
                            ${jobCards}
                            
                            <p>Don't miss out on these chances to accelerate your career.</p>
                        </div>
                        <div class="footer">
...
router.post("/scrape", authMiddleware, async (req, res) => {
    try {
        ...
        try {
            ...
            console.error("Remotive Error:", apiError.message);
        }
        ...
        if (newJobs.length > 0) {
            ...
            // Send Alert using the user object fetched earlier
            if (user && user.email) {
                sendJobAlertEmail(user.email, newJobs);
            }
