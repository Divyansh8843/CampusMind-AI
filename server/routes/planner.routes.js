...
const normalizeTopics = (syllabus = "") =>
    String(syllabus)
        .split(/[\n,;]+/)
...
router.post("/generate", authMiddleware, async (req, res) => {
    try {
        ...
        const modulesInput = normalizeTopics(syllabus);
        if (!modulesInput.length) {
            return res.status(400).json({ message: "Add at least one syllabus topic to generate a plan." });
        }
        ...
        if (Number.isNaN(end.getTime())) {
            return res.status(400).json({ message: "Provide a valid exam date." });
        }
        
        const days = Math.max(1, Math.ceil((end - start) / (1000 * 60 * 60 * 24)));
        if (end < start) {
            return res.status(400).json({ message: "Exam date must be today or later." });
        }
        
        // Topic Density Analysis & Distribution
        const tasks = [];
        const modules = modulesInput.map((name) => {
        ...
        modules.forEach((mod, idx) => {
            ...
            const spacingWindow = Math.max(1, days - 1);
            const dayOffset = Math.min(spacingWindow, Math.floor((idx / modules.length) * spacingWindow));
        ...
        if (days >= 3) {
            ...
            reviewDate.setDate(end.getDate() - 1);
            tasks.push({
                userId: req.user.userId,
                title: 'Final Revision & Mock Test',
                description: 'High-priority revision block automatically reserved before the exam.',
                deadline: reviewDate,
                type: 'exam',
                priority: 'critical',
                xpReward: 150
