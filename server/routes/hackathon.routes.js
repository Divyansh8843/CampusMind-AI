...
const sendHackathonAlertEmail = async (email, hackathonsList) => {
    if (!process.env.EMAIL_USER || !hackathonsList || hackathonsList.length === 0) return;
    try {
        const hackCards = hackathonsList.slice(0, 5).map(hack => `
            <div class="match-box" style="text-align: left; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin: 20px 0; border-left: 4px solid #7c3aed;">
                <h3 class="match-title" style="margin-top: 0; margin-bottom: 10px; color: #1e293b; font-size: 20px;">${hack.title}</h3>
                <div style="color: #64748b; font-weight: 600; margin-bottom: 5px;">🏆 ${hack.platform}</div>
                <div style="margin: 10px 0; font-size: 14px; color: #475569;">📅 ${hack.date}</div>
                <div style="margin-top: 15px;">
                    ${hack.tags.map(tag => `<span style="display: inline-block; background-color: #f3e8ff; color: #7e22ce; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 600; margin-right: 5px; margin-bottom: 5px;">${tag}</span>`).join('')}
                </div>
                <a href="${hack.link}" style="display: inline-block; margin-top: 15px; padding: 10px 16px; background-color: #7c3aed; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 14px; transition: background-color 0.3s;">View Hackathon</a>
            </div>
        `).join('');
        ...
        await transporter.sendMail({
            ...
            to: email,
            subject: `🔥 ${hackathonsList.length} New Hackathons Found!`,
            html: `
                <!DOCTYPE html>
                <html>
                <head>
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            <h2 style="color: #7c3aed; margin-top: 0;">New Hackathons Detected 🚀</h2>
                            <p>Hello,</p>
                            <p>Our intelligent AI agents have just scraped new hackathon opportunities. Here are the latest events:</p>
                            
                            ${hackCards}
                            
                            <p>Gather your squad and start building!</p>
                        </div>
                        <div class="footer">
                            <p>&copy; ${new Date().getFullYear()} CampusMind AI. All rights reserved.</p>
        ...
        console.error("Hackathon Alert Email failed:", error);
    }
};
    ...
    try {
        ...
        if (isCacheValid && hackathonCache.data.length > 0) {
            ...
            if (type && type !== 'All Type') {
                filteredCache = filteredCache.filter(h => h.tags.includes(type));
            }
        ...
        const fetchUnstop = async (type) => {
            try {
                ...
                return data.slice(0, 20).map(h => ({
                    ...
                    image: h.logoUrl?.url || h.banner_image || "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=800&auto=format&fit=crop",
                    platform: "Unstop",
                    location: isRemote ? "Remote" : "Global",
                    date: h.start_date ? new Date(h.start_date).toDateString() : "Open Now",
                    tags: ["Hackathon", isRemote ? "Remote" : "Global", ...(h.filters?.map(f => f.name) || [])]
                }));
            } catch (e) {
        ...
        const fetchDevpost = async () => {
            try {
                ...
                $('.hackathon-tile').each((i, el) => {
                    ...
                    let image = $(el).find('img').attr('src') || "https://upload.wikimedia.org/wikipedia/commons/thumb/6/69/Devpost_logo.svg/2560px-Devpost_logo.svg.png";
                    ...
                    if (title && link) {
                        hacks.push({
                            ...
                            location: isRemote ? "Remote" : "Global",
                            date: date || "Upcoming",
                            tags: [...tags, "Hackathon", isRemote ? "Remote" : "Global"]
                        });
                    }
                });
                return hacks;
        ...
        const [devpostData, unstopHacks] = await Promise.all([
            fetchDevpost(),
            fetchUnstop('hackathons')
        ]);

        globalOpportunities = [...devpostData, ...unstopHacks];
        ...
        globalOpportunities.push({
            ...
            location: "Remote",
            date: "Live Now",
            tags: ["Hackathon", "Remote", "Web3", "Emerging Tech"]
        });
        ...
        if (forceRefresh) {
            // Send Alert using the user object fetched via req.user
            const user = await User.findById(req.user.userId);
            if (user && user.email) {
                sendHackathonAlertEmail(user.email, globalOpportunities);
            }
        }
        ...
        if (type && type !== 'All Type') {
            filteredResults = filteredResults.filter(h => h.tags.includes(type));
        }
