...
router.post('/upload', authMiddleware, upload.single('syllabus'), async (req, res) => {
  try {
    ...
    let parsed;
    try {
      const aiRes = await callAiService('/parse-syllabus', { text: text.substring(0, 15000) }, { timeout: 30000 });
      parsed = aiRes.data;
    } catch (e) {
    ...
    let syllabus = await Syllabus.create({
...
// GET /api/syllabus - Get current user's syllabuses (real-time)
router.get('/', authMiddleware, async (req, res) => {
  try {
    const syllabuses = await Syllabus.find({ userId: req.user.userId }).sort({ createdAt: -1 });
    res.json({ success: true, syllabuses: syllabuses || [] });
  } catch (err) {
...
router.patch('/topic', authMiddleware, async (req, res) => {
  try {
    const { syllabusId, subjectIndex, topicIndex, completed } = req.body;
    const query = syllabusId ? { _id: syllabusId, userId: req.user.userId } : { userId: req.user.userId };
