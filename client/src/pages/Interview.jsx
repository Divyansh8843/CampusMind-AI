...
const Interview = () => {
  ...
  const INTERVIEWER_IMAGE = "/ai-interviewer.png";
      ...
      if (
        ...
        if (newCount > 2) {
          ...
          setWarningModal({
            ...
            message:
              "Maximum proctoring violations reached (3/3). Your session has been disqualified.",
          });
          logActivity(
            "Proctoring Disqualification",
            "User exceeded tab switch limit",
          );
  ...
  const startSession = async () => {
    ...
    try {
      ...
      if (mode === "mock" || mode === "salary") {
        ...
        const topic =
          mode === "salary"
            ? `Salary Negotiation - HR Roleplay for ${jobRole} targeting ${expectedSalary || "competitive"} LPA`
            : jobRole;
        logActivity(
          mode === "salary"
        ...
        const res = await axios.post(
          `${API_BASE_URL}/api/interview/aptitude`,
          { topic: jobRole },
          { headers: { Authorization: `Bearer ${token}` } },
        );
  ...
  const finishMockInterview = async (disqualified = false) => {
    ...
    try {
      if (disqualified) {
        setFeedback({
          score: 0,
          feedback: "Session Disqualified.",
          strengths: [],
          improvements: ["Adhere to integrity rules."],
        });
      } else {
        const token = localStorage.getItem("token");
        const topic =
      ...
      await axios.post(
        `${API_BASE_URL}/api/interview/save`,
        {
          ...
          feedback: disqualified
            ...
            : `Scored ${finalScore} out of ${aptitudeQuestions.length}.`,
        },
        { headers: { Authorization: `Bearer ${token}` } },
      );
    } catch (err) {
      ...
      {warningModal.show && (
        ...
        >
          ...
          >
            ...
            <p className="text-slate-600 dark:text-slate-300 font-medium whitespace-pre-line mb-8">
            ...
            {!isDisqualified ? (
              <button
                onClick={() => {
                  enterFullscreen();
                  setWarningModal({ ...warningModal, show: false });
                }}
  ...
  const renderIntro = () => (
    <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto mt-10">
      ...
      >
        ...
        <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-2xl flex items-center justify-center mb-6 text-blue-600 dark:text-blue-400 group-hover:scale-110 transition-transform">
          <Brain size={32} />
        </div>
        <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">
          FAANG MNC Aptitude Test
        </h3>
        <p className="text-slate-500 dark:text-slate-400 mb-6">
          100% rigorous global-standard MNC assessment powered by AI.
        </p>
      ...
      >
        ...
        <div className="w-16 h-16 bg-purple-100 dark:bg-purple-900/30 rounded-2xl flex items-center justify-center mb-6 text-purple-600 dark:text-purple-400 group-hover:scale-110 transition-transform">
          <Mic size={32} />
        </div>
        <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">
        ...
        <p className="text-slate-500 dark:text-slate-400 mb-6">
          Interactive FAANG-level AI video interview testing deep domain
      ...
      >
        ...
        <div className="w-16 h-16 bg-amber-100 dark:bg-amber-900/30 rounded-2xl flex items-center justify-center mb-6 text-amber-600 dark:text-amber-400 group-hover:scale-110 transition-transform">
          <Trophy size={32} />
        </div>
        <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">
        ...
        <p className="text-slate-500 dark:text-slate-400 mb-6">
          Master HR negotiations. AI simulates a 'Tough HR' to help freshers and
          experienced pros secure top offers.
        </p>
        <div className="flex items-center gap-4 text-sm text-slate-500 mb-6 font-mono">
          <span className="flex items-center gap-1">
            <Mic size={14} /> Voice
    ...
    <div className="max-w-7xl mx-auto h-[calc(100vh-120px)] flex flex-col gap-4">
      ...
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-2 text-slate-500 font-mono text-sm">
          <span
            className={`w-3 h-3 rounded-full ${isDisqualified ? "bg-red-500" : "bg-green-500 animate-pulse"}`}
          ></span>
          AGENTIC AI LIVE
        </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-6 relative p-4">
        {/* AI Interviewer - RECTANGULAR BOX */}
        {/* AI Interviewer Panel */}
        <div className="w-full lg:w-[65%] aspect-video bg-gradient-to-br from-slate-900 via-slate-950 to-black rounded-3xl relative overflow-hidden border border-slate-700 shadow-2xl group flex items-center justify-center">
          {/* AI Avatar */}
          <img
            ...
            ${
              isSpeaking
                ? "scale-105 drop-shadow-[0_0_40px_rgba(168,85,247,0.5)]"
                : "scale-100"
            }
        `}
          />
          ...
          {isSpeaking && (
            <>
              ...
              <div className="absolute inset-0 rounded-3xl border-4 border-purple-500/40 animate-pulse pointer-events-none"></div>
            </>
          )}

          {/* Top Left Badge */}
          <div className="absolute top-4 left-4 z-20">
            <div className="bg-black/70 backdrop-blur-xl border border-white/10 px-4 py-2 rounded-full flex items-center gap-2 text-white text-xs font-bold">
              <Cpu size={14} />
              AI INTERVIEWER
            </div>
          </div>
          ...
          <div className="absolute top-4 right-4 z-20">
            <div className="bg-black/60 backdrop-blur-xl border border-white/10 px-3 py-2 rounded-full text-white/80 text-xs flex items-center gap-2">
              <Sparkles size={12} />
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20">
            ...
            >
              ...
              <span className="font-semibold tracking-wide">
                {isSpeaking ? "AI SPEAKING" : "READY TO INTERVIEW"}
              </span>
            </div>
          </div>
          ...
          <div className="absolute bottom-6 right-6 z-20">
            <div className="flex items-center gap-2 bg-red-500/90 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg">
        ...
        <div className="w-full lg:w-[35%] aspect-video bg-black rounded-3xl relative overflow-hidden border border-slate-700 shadow-2xl flex items-center justify-center group">
          {!hasVideo && (
          ...
          <video
            ...
            muted
            playsInline
            className={`w-full h-full object-cover transform scale-x-[-1] transition-opacity duration-700 ${hasVideo && !isVideoOff ? "opacity-100" : "opacity-0"}`}
          />
          {isVideoOff && (
          ...
          {hasVideo && !isVideoOff && (
            <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md p-3 rounded-xl border border-white/10 text-xs font-mono text-cyan-400 space-y-1 z-20">
              ...
              <div className="flex justify-between gap-4">
                ...
                <span
                  className={
                    facialStats.confidence > 90
                      ? "text-green-400"
                      : "text-yellow-400"
                  }
                >
                  {facialStats.confidence}%
                </span>
              </div>
              <div className="flex justify-between gap-4">
                ...
                <span
                  className={
                    facialStats.eyeContact ? "text-green-400" : "text-red-400"
                  }
                >
                  {facialStats.eyeContact ? "MATCH" : "LOST"}
                </span>
              </div>
              <div className="flex justify-between gap-4">
                ...
                <span
                  className={
                    facialStats.smile ? "text-green-400" : "text-slate-400"
                  }
                >
                  {facialStats.smile ? "POSITIVE" : "NEUTRAL"}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/50 backdrop-blur-md p-2 rounded-xl border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              onClick={toggleMic}
              className={`p-2 rounded-lg ${isMicMuted ? "bg-red-500 text-white" : "bg-white/10 text-white hover:bg-white/20"}`}
            >
              {isMicMuted ? <MicOff size={16} /> : <Mic size={16} />}
            </button>
            <button
              onClick={toggleVideo}
              className={`p-2 rounded-lg ${isVideoOff ? "bg-red-500 text-white" : "bg-white/10 text-white hover:bg-white/20"}`}
            >
          ...
          {timer > 120 && (
            <div className="absolute inset-x-0 bottom-20 text-center">
              <span className="bg-red-600 text-white px-4 py-2 rounded-full font-bold animate-bounce shadow-lg">
                ⚠️ 2:00 Limit
              </span>
            </div>
          )}
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          {!isListening ? (
            <button
              ...
              disabled={loading || isSpeaking || isDisqualified}
          ...
          <button
            onClick={() => finishMockInterview(false)}
            className="px-6 py-4 rounded-xl bg-red-50 dark:bg-red-900/10 text-red-600 font-bold hover:bg-red-100 dark:hover:bg-red-900/20 transition-all"
          >
            End
          </button>
        </div>
  ...
  const renderResult = () => (
    <div className="max-w-4xl mx-auto mt-10 space-y-8 pb-10">
      <div className="text-center">
        <div
          className={`inline-block p-4 rounded-full mb-4 ${isDisqualified ? "bg-red-100 text-red-600" : "bg-yellow-100 text-yellow-600"}`}
        >
        ...
        <div
          className={`text-6xl font-black mt-4 ${isDisqualified ? "text-red-500" : "text-blue-600 dark:text-blue-400"}`}
        >
          {mode === "aptitude"
            ? `${Math.round(aptitudeScore)}/${aptitudeQuestions.length}`
            : `${feedback?.score || 0}/100`}
        </div>
      </div>
      {!isDisqualified &&
        (mode === "mock" || mode === "salary") &&
        feedback && (
          <div className="space-y-6">
            {feedback.voice_analysis && (
              <div className="bg-purple-50 dark:bg-purple-900/20 p-6 rounded-2xl border border-purple-200 dark:border-purple-900/30">
                <h3 className="font-bold text-purple-600 dark:text-purple-400 mb-4 flex items-center gap-2">
                  <Mic size={20} /> Voice Analysis (AI Voice Interviewer)
                </h3>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-purple-600">
                      {feedback.voice_analysis.filler_count || 0}
                    </span>
                  </div>
                  {feedback.voice_analysis.fillers &&
                    feedback.voice_analysis.fillers.length > 0 && (
                      <div className="mt-2">
                        <span className="text-slate-600 dark:text-slate-300">
                          Breakdown:{" "}
                        </span>
                        {feedback.voice_analysis.fillers.map((f, i) => (
                          <span
                            key={i}
                            className="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 rounded text-xs mr-1"
                          >
                            "{f.word}" ({f.count}x)
                          </span>
                        ))}
            ...
            {aptitudeQuestions.map((q, i) => (
              ...
              >
                ...
                <div className="flex justify-between text-sm">
                  <span
                    className={`${userAnswers[q.id] === q.correct_answer ? "text-green-600" : "text-red-500"}`}
                  >
                    You: {userAnswers[q.id]}
                  </span>
      ...
      <div className="flex justify-center">
        <button
          ...
          className="px-8 py-3 bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl font-bold"
        >
          Return to Dashboard
        </button>
      </div>
  ...
  const renderSetup = () => (
    <div className="max-w-md mx-auto mt-20 bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 dark:border-white/10 shadow-2xl text-center relative">
      <button
        ...
        className="absolute top-4 left-4 p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-full"
      >
        <ArrowLeft size={20} />
      </button>
      <div
        className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${mode === "mock" ? "bg-purple-100 text-purple-600" : mode === "salary" ? "bg-amber-100 text-amber-600" : "bg-blue-100 text-blue-600"}`}
      >
        {mode === "mock" ? (
          <Mic size={40} />
        ) : mode === "salary" ? (
      ...
      <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">
        {mode === "salary" ? "Target Role & Salary" : "Select Role"}
      </h2>
        ...
        {mode === "salary" ? (
          <>
            <div>
              ...
              <input
                ...
                placeholder="e.g. SDE-2 or Senior Developer"
                className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 outline-none font-medium text-slate-800 dark:text-white"
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1 dark:text-slate-300">
          ...
          <div>
            ...
            <select
              value={jobRole}
              onChange={(e) => setJobRole(e.target.value)}
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-4 outline-none font-medium text-slate-800 dark:text-white"
            >
      ...
      <button
        ...
        disabled={loading}
        className={`w-full mt-8 py-4 text-white rounded-xl font-bold text-lg shadow-lg flex items-center justify-center gap-2 ${mode === "mock" ? "bg-purple-600" : mode === "salary" ? "bg-amber-600" : "bg-blue-600"}`}
      >
        {loading ? "Initializing Agent..." : "Begin Session"}{" "}
        <ArrowRight size={20} />
      </button>
    </div>
  );
  ...
  return (
    <div
      className="max-w-7xl mx-auto min-h-[calc(100vh-100px)] p-4 relative font-sans select-none"
            ...
            <div className="max-w-4xl mx-auto bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 dark:border-white/10 shadow-2xl relative">
              ...
              <div className="flex justify-between items-center mb-8 border-b border-slate-100 dark:border-white/5 pb-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <Brain size={24} className="text-blue-500" /> Aptitude:{" "}
                    {jobRole}
                  </h2>
                ...
                <div
                  className={`px-4 py-2 rounded-xl flex items-center gap-2 font-mono font-bold text-xl shadow-inner ${aptitudeTimer < 30 ? "bg-red-500 text-white animate-pulse" : "bg-slate-100 dark:bg-black/30 text-slate-700 dark:text-slate-300"}`}
                >
                  <Clock size={20} /> {formatTime(aptitudeTimer)}
                </div>
              </div>
              <div className="mb-8 min-h-[300px]">
                <h3 className="text-xl font-medium text-slate-800 dark:text-white mb-8 leading-relaxed">
                  {aptitudeQuestions[currentQuestionIndex]?.question}
                </h3>
                <div className="grid gap-4">
                  {aptitudeQuestions[currentQuestionIndex]?.options.map(
                    (opt, idx) => (
                      <button
                        key={idx}
                        onClick={() =>
                          setUserAnswers({
                            ...userAnswers,
                            [aptitudeQuestions[currentQuestionIndex].id]: opt,
                          })
                        }
                        className={`w-full text-left p-4 rounded-xl border-2 transition-all flex items-center justify-between group hover:shadow-md ${userAnswers[aptitudeQuestions[currentQuestionIndex].id] === opt ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20" : "border-slate-100 dark:border-white/5 hover:border-blue-200 dark:hover:border-blue-500/30"}`}
                      >
                        <span className="font-medium text-slate-700 dark:text-slate-300 text-lg">
                          {opt}
                        </span>
                        {userAnswers[
                          aptitudeQuestions[currentQuestionIndex].id
                        ] === opt && (
              ...
              <div className="flex justify-between pt-6 border-t border-slate-100 dark:border-white/5">
                <button
                  ...
                  className="px-6 py-2 text-slate-500 hover:text-slate-800 dark:hover:text-white disabled:opacity-30 transition-colors font-bold flex items-center gap-2"
                >
                ...
                {currentQuestionIndex < aptitudeQuestions.length - 1 ? (
                  <button
                    onClick={() => setCurrentQuestionIndex((p) => p + 1)}
                    className="px-8 py-3 bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl font-bold hover:transform hover:-translate-y-1 transition-all flex items-center gap-2 shadow-lg"
                  >
                    Next <ArrowRight size={16} />
                  </button>
                ) : (
                  <button
                    onClick={() => submitAptitude(false)}
                    className="px-8 py-3 bg-green-600 text-white rounded-xl font-bold hover:transform hover:-translate-y-1 transition-all shadow-lg shadow-green-500/30 flex items-center gap-2"
                  >
