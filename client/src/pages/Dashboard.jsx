...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const finalize = async () => {
            const token = localStorage.getItem('token');
            try {
                // Verify payment immediately so the UI updates without waiting for webhook timing.
                if (sessionId && token) {
                    setFinalizingPayment(true);
                    // Stripe status can be slightly delayed; retry a few times.
                    for (let attempt = 0; attempt < 8; attempt++) {
                        try {
                            ...
                            const isNotPaidYet = status === 400;
                            if (!isNotPaidYet || attempt === 7) throw e;
                ...
                // Fall back to normal user refresh (webhook may have already updated MongoDB).
            } finally {
                await refreshUser();
        ...
        >
            {finalizingPayment && (
                <div className="fixed inset-0 z-50 bg-slate-900/60 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-xl max-w-md w-full">
                        <div className="text-lg font-bold text-slate-900 dark:text-white">Finalizing your Premium...</div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                <h3 className="text-xl font-bold mb-1">Alumni Network</h3>
                                <p className="text-slate-500 dark:text-slate-400 text-sm">Connect & get mentored.</p>
                            </div>
                        </NavLink>

                        <NavLink 
                            to="/syllabus" 
                            className="group bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/5 text-slate-800 dark:text-white rounded-2xl p-6 hover:bg-slate-50 dark:hover:bg-slate-750 hover:border-emerald-300 dark:hover:border-emerald-500/30 hover:scale-[1.02] transition-all relative overflow-hidden shadow-sm dark:shadow-none"
                        >
                            <div className="flex justify-between items-start mb-8 relative z-10">
                            ...
                            <div className="relative z-10">
                                <h3 className="text-xl font-bold mb-1">Syllabus Tracker</h3>
                                <p className="text-slate-500 dark:text-slate-400 text-sm">Live academic progress.</p>
                            </div>
                        </NavLink>
                    </div>
